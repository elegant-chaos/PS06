...
squared_bias <- estimates %>% summarise(mean_df2 = mean(estimates_df2), 
  ...
  mutate(bias_df2 = (0.95 - mean_df2)^2,bias_df99 = (0.95 - mean_df99)^2) %>% 
...
Note: The difference between MSE and sum is a result of the randomness created by using the rnorm() function to simulate the noise in the data set. However, since the sum falls within the range of 1 $\sigma^2 = 0.09$, the results displayed in the table remain reasonable.
...
2. The process of finding the MSE for all values would look very similar to the process used here. Except, instead of finding the MSE for $x_0=0.95$, we'd find the error using the mean(y_observed) compared to the $mean(f(x)+ \epsilon)$ over the range of possible x values. 
